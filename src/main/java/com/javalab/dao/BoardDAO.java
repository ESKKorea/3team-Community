...
//			sql.append("where a.row_num between ? and ?");
	...
	public BoardVO getPreviousBoard(int bno) throws SQLException {
		...
		try {
			conn = dataSource.getConnection();
			String sql = "SELECT bno, title FROM board WHERE bno < ? ORDER BY bno DESC FETCH FIRST 1 ROWS ONLY";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bno);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				previousBoard = new BoardVO();
				previousBoard.setBno(rs.getInt("bno"));
				previousBoard.setTitle(rs.getString("title"));
	...
	public BoardVO getNextBoard(int bno) throws SQLException {
		...
		try {
			conn = dataSource.getConnection();
			String sql = "SELECT bno, title FROM board WHERE bno > ? ORDER BY bno FETCH FIRST 1 ROWS ONLY";
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, bno);
			rs = pstmt.executeQuery();
			if (rs.next()) {
				nextBoard = new BoardVO();
				nextBoard.setBno(rs.getInt("bno"));
				nextBoard.setTitle(rs.getString("title"));
				// Set other properties as needed
			}
		} finally {
			closeResource();
	...
	public BoardVO getBoard(int bno) {
		...
		try {
			...
			String sql = "select bno, title, content, member_id, reg_date, hit_no, "
					+ "reply_group, reply_order, reply_indent " + "from board where bno=? ";
