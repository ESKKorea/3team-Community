...
public class BoardDAO {
	...
	// BoardDAO를 싱클톤 패턴으로 단 한개의 객체만 생성하기 위한 변수 선언
	private static BoardDAO instance;

	/**
	 * private 생성자 - 밖에서는 절대로 호출할 수 없다. 즉, 밖에서 객체 생성 불가
	 */
	private BoardDAO() {
		try {
			Context ctx = new InitialContext();
			dataSource = (DataSource) ctx.lookup("java:comp/env/jdbc/oracle"); // 커넥션 풀
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	/**
	 * BoardDAO 자신의 인스턴스(객체)를 반환해주는 메소드 - 이메소드는 최초로 호출 될 때는 아직 객체가 생성되어 있지 않으므로 그때 단
	 * 한번 객채로 생성된다. - 그 다음 부터는 이미 생성되어 있는 그 객체의 참조 주소값을 반환한다. - static 객체 생성 없이도 밖에서
	 * 호출할 수 있도록 하기 위해서
	 * 
	 * @return
	 */
	...
	public int insertBoard(BoardVO boardVO) {
		...
		try {
			...
			// 새글 등록시 사용할 bno, reply_group 번호 조회
			...
			// 2. 위에서 얻은 nextVal사용하여 bno, reply_group 세팅 
	 ...
	 * - 답변 게시물 등록 작업
	 */
	public int insertReflyBoard(BoardVO boardVO) {
		int row = 0;
		try {
			conn = dataSource.getConnection();
			conn.setAutoCommit(false); // 트랜잭션 시작 (오토커밋)
			...
			pstmt.setInt(4, boardVO.getReplyGroup()); // 부모의 reply_group을 넣어줌(그래야 부모와 묶임)
			pstmt.setInt(5, boardVO.getReplyOrder()+ 1); // 부모의 reply_order +1을 넣어줌(그래야 부모 바로 밑에 위치함)
			pstmt.setInt(6, boardVO.getReplyIndent() +1); // 부모의 reply_indent을 넣어줌(부모보다 한칸더 들여써짐)
	 ...
	 * ArrayList<>(); try { conn = dataSource.getConnection(); String sql =
	 * "SELECT bno, title, content, member_id, reg_date, hit_no FROM board ORDER BY bno DESC"
	 * ; pstmt = conn.prepareStatement(sql); rs = pstmt.executeQuery(); while
	 * (rs.next()) { BoardVO boardVO = new BoardVO();
	 * 
	 ...
	 * (Exception e) { e.printStackTrace(); // 콘솔 오류
	 * System.out.println("getBoardList ERR : " + e.getMessage()); } finally {
	 * closeResource(); // 자원 해제 } return boardList; }
	 */
	public List<BoardVO> getBoardList(BoardVO boardVO) {
		 ...
		 *	[첫 게시물과 끝 게시물 구하는 공식]
		 *1. row_number()
		 * - 시작 게시물 번호:(사용자가 요청한 페이지 -1) * 한페이지보여줄 게시물수 + 1
		 * - 끝 게시물 번호 : 시작번호 + 한페이지에 보여줄 게시물 수 -1.
		 * 2. Fetch ~ Next 구문 사용
		 * - 건너뛸 게시물수:(사용자가 요청한 페이지 -1) * 한페이지보여줄 게시물수 
		...
		// 가져올 게시물수
		end = start + boardVO.getListCount();
		...
		try {
			 ...
			 * ); sql.append("from board b "); sql.append(")a ");
	...
	public List<BoardVO> searchBoardList(String keyword) {
		...
		try {
			...
			while (rs.next()) {
				...
				boardVO.setReplyGroup(rs.getInt("reply_group"));
				boardVO.setReplyOrder(rs.getInt("reply_order"));
				boardVO.setReplyIndent(rs.getInt("reply_indent"));
				boardList.add(boardVO);
			}
		} catch (Exception e) {
			e.printStackTrace(); // 콘솔 오류
			System.out.println("getBoardList ERR : " + e.getMessage());
		} finally {
			closeResource(); // 자원 해제
		}
	...
	public int deleteBoard(int bno) {
		...
		try {
			conn = dataSource.getConnection();
			String sql = "delete board where bno = ?"; // 동적 파라미터
			pstmt = conn.prepareStatement(sql); // PreparedStatement 객체 얻기(쿼리문 전달)
	...
	/* 베스트 댓글 */
	public List<BoardVO> getBestComments() {
	    List<BoardVO> bestComments = new ArrayList<>();

	    try {
	        conn = dataSource.getConnection();
	        String sql = "SELECT bno, title, content, member_id, reg_date, hit_no, reply_group, reply_order, reply_indent " +
	                     "FROM board " +
	                     "ORDER BY hit_no DESC, reg_date DESC " +
	        ...
	        while (rs.next()) {
	            ...
	            bestComments.add(board);
	        }
	    } catch (SQLException e) {
	        System.out.println("getBestComments ERR : " + e.getMessage());
	        e.printStackTrace();
	    } finally {
	        closeResource();
	    }
