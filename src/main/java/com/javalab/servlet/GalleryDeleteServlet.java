...
public class GalleryDeleteServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 파라미터 인코딩
				...
				int row = galleryDAO.deleteGallery(bno);
				
				// 데이터베이스 작업 결과로 분기
				if(row > 0) {
					String contextPath = request.getContextPath();
					response.sendRedirect(contextPath + "/galleryList");
				}else {
					request.setAttribute("error", "게시물 삭제에 실패했습니다.");
					RequestDispatcher rd = request.getRequestDispatcher("/galleryDetail?bno=" + bno);
