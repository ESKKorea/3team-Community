...
public class PageNavigator {
   ...
   public String getPageNavigator(int totalCount, int listCount, int pagePerBlock,
         int pageNum) {
	   System.out.println("pageNavihator : " + totalCount + " / " + listCount + " / " + pagePerBlock + " / " + pageNum);
      ...
      if(totalCount > 0) {   //총 레코드 수가 하나라도 있어야 페이징
    	  /***
    	   * [총 페이지수]
    	   * -  총게시물이 101개이고 한페이지에 10개의 페이지번호를 노출시켜야 된다는 가정.
    	   * - 101 % 10 = 1이 됨므로 총 페이지는 11이 됨.
    	   */
         int totalNumOfPage = (totalCount % listCount == 0) ? totalCount / listCount :   totalCount / listCount + 1;
         
         /***
          * [총 페이지 블럭]
          * - 전체 페이지가 11개이고 한 페이지에 10개의 페이지번호를 표시한다고 가정하면 총 페이지블럭 2개가 됨.
          * - 11 % 10 != 0 이면 페이지 블럭은 2가 된다. [1..10] [11..20]
          */
         ...
         int currentBlock = (pageNum % pagePerBlock == 0) ? 
               ...
               pageNum / pagePerBlock + 1; //2 페이지 블럭 ([11..20])
         
         System.out.println("currentBlock : " + currentBlock);
         
         /**
          *  시작페이지 번호 : (현재 페이지블럭 -1) * 10 + 1 =1 
          */
          ...
          * - 실제 페이지 갯수가 11인데 계산 공식에 의해서 끝페이지는 20이다
          * - 이럴경우에는 실제 페이지 개수인 11을 끝페이지로 한다.
          */
         ...
         /**
          * 다음이 페이지가 있는지 표시 > >>
          * - 현재 페이지블럭(1) < 전체페이지 블럭(2) 표식을 노출시킨다.
          */
         ...
         /**
          *  총 페이지 블럭이 단 하나인 경우 뒤로가기 / 앞으로가  표식 노출 안시킴.
          */
