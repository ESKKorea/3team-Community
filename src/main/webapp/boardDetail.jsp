	...
	<div class="nav">
		...
		<div class="nav_but">
			<a href="${contextPath}/boardList">자유게시판</a> <a
				href="${contextPath}/galleryList">갤러리 게시판</a> <a href="">분양 게시판</a>
			<c:choose>
	...
	<div class="prev-next-section">
		<c:if test="${not empty previousBoard}">
			<div class="previous">
				...
				<a href="<c:url value='/boardDetail'/>?bno=${previousBoard.bno}"
					class="button">이전 글</a>
			</div>
		</c:if>
		...
		<c:if test="${not empty nextBoard}">
			<div class="next">
				...
				<a href="<c:url value='/boardDetail'/>?bno=${nextBoard.bno}"
					class="button">다음 글</a>
			</div>
		</c:if>
	</div>
